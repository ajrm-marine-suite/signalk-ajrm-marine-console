/**
 * Signal K server entry point for AJRM Marine Console; registers lifecycle, subscriptions, routes, and status.
 */
...
module.exports = function ajrmMarineConsole(app) {
  ...
  function registerRoutes(router, prefix = "") {
    ...
    if (typeof router.post === "function") {
      ...
      router.post(`${prefix}/bite/run-group`, async (req, res) => {
        try {
          const report = await bite.runGroup(req.body || {});
          res.set?.("Cache-Control", "no-store");
          res.status?.(200);
          res.json(report);
        } catch (error) {
